...
public class TileCrafter extends UpgradeableTileEntity {
    ...
    public boolean canCraft() {
        return ((slots[OUTPUT_SLOT].isEmpty()) ||
                ((slots[OUTPUT_SLOT].isItemEqual(slots[CRAFT_RESULT])) && ((slots[OUTPUT_SLOT].getCount()
                        + slots[CRAFT_RESULT].getCount()) <= slots[OUTPUT_SLOT].getMaxStackSize())));
    }
            ...
            if (!slots[i].isEmpty()) {
                ...
                if (consume) {
                    if (amtItems <= 0) {
                        ...
                        if ((!slots[i].isEmpty()) && (slots[i].getCount() != amtItems)) {
                            if (slots[i].getItem().hasContainerItem(slots[i])) {
                                ItemStack container = slots[i].getItem().getContainerItem(slots[i]);
    ...
    public boolean canExtractItem(int slot, ItemStack stack, EnumFacing side) {
        ...
        if (sides[side.ordinal()] == WrenchModes.Mode.Disabled) {
            return false;
        }
