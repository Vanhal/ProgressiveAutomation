...
import com.vanhal.progressiveautomation.common.util.CoordList;
...
public class TileChopper extends UpgradeableTileEntity {
    ...
    public void readNonSyncableNBT(NBTTagCompound nbt) {
        ...
        if (removeShears) {
            if (!slots[SLOT_SHEARS].isEmpty()) {
                slots[SLOT_SHEARS] = ItemStack.EMPTY;
            }
    ...
    public void readSyncOnlyNBT(NBTTagCompound nbt) {
        ...
        if (nbt.hasKey("chopping")) {
            chopping = nbt.getBoolean("chopping");
        }
        if (nbt.hasKey("planting")) {
            plantSapling = nbt.getBoolean("planting");
        }
    }
    ...
    protected boolean scanBlocks() {
        ...
        if (blockList.size() > 0) {
            return true; //if we can harvest some trees lets do that before planting more
        }
    ...
    protected boolean isTree(int x, int y, int z) {
        return isTree(new BlockPos(x, y, z));
    }
    ...
    protected boolean isLeaf(int x, int y, int z) {
        return isLeaf(new BlockPos(x, y, z));
    }
            ...
            if (slots[SLOT_SAPLINGS].getCount() > 0) {
                ...
                if (Block.getBlockFromItem(slots[SLOT_SAPLINGS].getItem()) instanceof IPlantable) {
                    ...
                    if ((tree.canPlaceBlockAt(world, plantPos)) &&
                        ...
                        if (doAction) {
                            world.setBlockState(plantPos, tree.getStateFromMeta(slots[SLOT_SAPLINGS].getItem().getDamage(slots[SLOT_SAPLINGS])), 7);
