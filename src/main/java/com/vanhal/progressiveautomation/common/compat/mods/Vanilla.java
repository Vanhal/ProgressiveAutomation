...
public class Vanilla extends BaseMod {
    ...
    public boolean isPlantible(ItemStack item) {
        ...
        if ((item.getItem() == Items.DYE) && (EnumDyeColor.byDyeDamage(item.getMetadata()) == EnumDyeColor.BROWN)) {
            return true; //Coco beans
        }

        if (item.getItem() == Items.REEDS) {
            return true; // sugar cane
        }

        if (Block.getBlockFromItem(item.getItem()) == Blocks.CACTUS) {
            return true; // cactus
        }
        return false;
    }
    ...
    public boolean validBlock(World worldObj, ItemStack itemStack, Point3I point) {
        ...
        if (plant != null) {
            return (plant.getBlock().canPlaceBlockAt(worldObj, point.toPosition())) && (worldObj.getBlockState(point.toPosition()).getBlock() != plant.getBlock());
        }
        return false;
    }
