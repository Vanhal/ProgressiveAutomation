...
public class ContainerCrafter extends BaseContainer {
    ...
    public ContainerCrafter(InventoryPlayer inv, TileEntity entity) {
        ...
        init = true;
        addSlotToContainer(new SlotCraftingLocked(inv.player, craftMatrix, crafter, crafter.CRAFT_RESULT, 69, 34));
        addCraftingGrid(craftMatrix, 0, 10, 16, 3, 3); //crafting grid
        addSlotToContainer(new SlotRemoveOnly(crafter, crafter.OUTPUT_SLOT, 150, 16));
        //output slots
    ...
    public void onCraftMatrixChanged(IInventory inv) {
    	if (CraftingManager.findMatchingRecipe(this.craftMatrix, crafter.getWorld())!=null) {
    		crafter.setInventorySlotContents(crafter.CRAFT_RESULT, CraftingManager.findMatchingRecipe(this.craftMatrix, crafter.getWorld()).getRecipeOutput());
    	}
        if (init) {
            updateTile();
        }
